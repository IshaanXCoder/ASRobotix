  ...
  return (
    <section id="pricing" className="py-16 px-4 sm:px-6">
      <div className="max-w-4xl mx-auto">
        <h2 className="text-3xl sm:text-4xl font-bold mb-4 text-center">Simple, Transparent Pricing</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {plans.map((plan) => (
            <div key={plan.name} className="card-hover border-hover">
              <ShineBorder
                className={`h-full ${plan.highlight ? "z-10" : ""}`}
                borderClassName={`border ${plan.highlight ? "border-red-500/50" : "border-white/10"} rounded-xl`}
              >
                <div className={`p-6 sm:p-8 h-full ${plan.highlight ? "bg-gradient-to-b from-black to-black/80" : ""}`}>
                  <div className="mb-8">
                    <h3 className="text-xl sm:text-2xl font-bold mb-2">{plan.name}</h3>
                    <div className="flex items-end gap-1 mb-2">
                      <span className="text-3xl sm:text-4xl font-bold">{plan.price}</span>
                      {plan.period && <span className="text-gray-400 mb-1 text-sm sm:text-base">{plan.period}</span>}
                    </div>
